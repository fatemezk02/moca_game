...
export const Gallery07View: React.FC<Gallery07ViewProps> = ({
  ...
  const [arrows, setArrows] = useState<AdminArrowPoint[]>(() => getGalleryArrows('gallery-07'));
  ...
  useEffect(() => {
    const handleUpdate = () => {
      setPoints(getGalleryPoints('gallery-07'));
    };
  ...
  useEffect(() => {
    const handleArrowsUpdate = () => {
      setArrows(getGalleryArrows('gallery-07'));
    };
  ...
  const effectiveCollectionPoints = useMemo(() => {
    const configuredCollectionPoints = points.filter(
      (p): p is AdminCollectionPoint =>
        p.type === 'collection' &&
        !['star-16', 'star-17', 'star-18'].includes(p.id) &&
        !['star-16', 'star-17', 'star-18'].includes(p.starId || '')
    );
