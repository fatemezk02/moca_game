...
interface PuzzlePointModalProps {
  galleryId: string;
...
export const PuzzlePointModal: React.FC<PuzzlePointModalProps> = ({
  galleryId,
  puzzlePoint,
  onClose,
}) => {
  ...
  const isAllInitiallyComplete = isGalleryPuzzleCompleted(galleryId);
  ...
  const [collectedPieces, setCollectedPieces] = useState<string[]>(() =>
    getCollectedPiecesForGallery(galleryId)
  );
  ...
  const startAssemblySequence = () => {
    ...
    setTimeout(() => {
      markGalleryPuzzleCompleted(galleryId);
      triggerCelebration();
