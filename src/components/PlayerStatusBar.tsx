...
import { GlobalGuideAudioPlayer } from './GlobalGuideAudioPlayer';
...
export const PlayerStatusBar: React.FC<PlayerStatusBarProps> = ({
  ...
  return (
    <div className="w-full flex flex-col select-none shrink-0 z-30">
      <div
        id="player-status-bar"
        role="region"
        aria-label="وضعیت بازیکن"
        className={`w-full px-3 sm:px-6 py-1.5 flex items-center justify-between pointer-events-none ${className}`}
      >
      ...
      {/* Global Sticky Audio Bar directly attached under the HUD */}
      <GlobalGuideAudioPlayer />
    </div>
  );
};
