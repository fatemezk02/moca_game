...
interface Gallery03QuestionsViewProps {
  onNavigateBack: () => void;
...
export const Gallery03QuestionsView: React.FC<Gallery03QuestionsViewProps> = ({
  onNavigateBack,
  onSelectTab,
}) => {
  const isAlreadyCompleted = isGalleryQuestionsCompleted('gallery-03');
  ...
  // Configured artwork settings for Gallery 03
  const [artworkConfig, setArtworkConfig] = useState<GalleryQuestionsArtworkConfig>(() =>
    getGalleryQuestionsArtwork('gallery-03')
  );
  ...
  useEffect(() => {
    const handleUpdate = (e: Event) => {
      ...
      if (customEvent.detail?.galleryId === 'gallery-03') {
  ...
  const handleSelectOption = (_optionIndex: number) => {
    ...
    const currentQ = GALLERY_03_QUESTIONS[currentQuestionIndex];
    if (currentQ) {
      markQuestionAnswered(`gallery03-q0${currentQ.id}`);
      markQuestionAnswered(`gallery03-q${currentQ.id}`);
    }

    if (currentQuestionIndex === 0) {
      // After Question 1 -> show 1/3 progress artwork
      setGalleryAnsweredCount('gallery-03', 1);
      setProgressStep(1);
      setPhase('progress');
    } else if (currentQuestionIndex === 1) {
      // After Question 2 -> show 2/3 progress artwork
      setGalleryAnsweredCount('gallery-03', 2);
      setProgressStep(2);
      setPhase('progress');
    } else {
      // After Question 3 -> transition to final completed artwork view
      setGalleryQuestionsCompleted('gallery-03', true);
      setPhase('completed');
      lockTimerRef.current = setTimeout(() => {
        setIsLocked(false);
      }, 450);
  ...
  const currentQuestion = GALLERY_03_QUESTIONS[currentQuestionIndex] || GALLERY_03_QUESTIONS[0];

  return (
    <div
      id="gallery-03-questions-root"
    ...
    >
      {/* Top App Bar Header */}
      <header
        id="gallery-03-questions-top-bar"
          ...
          {phase === 'progress' && (
            ...
            >
              ...
              >
                ...
                <motion.img
                  src={artworkConfig.image || GALLERY_01_ARTWORK_SRC}
                  alt="Gallery 03 Artwork - Bottom Third"
                ...
                {progressStep === 2 && (
                  <motion.img
                    src={artworkConfig.image || GALLERY_01_ARTWORK_SRC}
                    alt="Gallery 03 Artwork - Middle Third"
                    initial={{ opacity: 0 }}
